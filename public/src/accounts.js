...
function sortAccountsByLastName(accounts) {
  ...
  let sortedResults = accounts.sort((account1, account2) => (account1.name.last.toLowerCase() < account2.name.last.toLowerCase()) ? -1 : 1);
...
function getAccountFullNames(accounts) {
  ...
  let fullNames = accounts.map((account) => { 
    ...
    let last = account.name.last;
    name.push(`${first} ${last}`);
  })
  return name;
